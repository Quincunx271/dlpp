...
{
    dlpp::dl lib(
        "libnonexistantlibrarythatisverylongandcannotexistfordlpptestcase.so", dlpp::dl_flags::now);

    REQUIRE_FALSE(lib);
    CHECK_THAT(dlpp::dl_error_message(),
...
TEST_CASE("Loading nonexistent symbol has nice error")
{
    ...
    REQUIRE_FALSE(lib.sym<int()>("getCounter2"));
    CHECK_THAT(dlpp::dl_error_message(), Contains("libcounter.so") && Contains("getCounter2"));
}
